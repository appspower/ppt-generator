...
**PPTAgent (EMNLP 2025) 실제 프롬프트 분석**:
- 레이아웃 선택 프롬프트 = ~35줄, 플랫 리스트 + 1개 예시 + "Content Fit 평가" 지시
- Decision tree 없음, 스코어링 없음
- 다양성 보장 = `shuffle(layouts)` — 프롬프트에 "반복 금지"라는 지시조차 없음
- 5개 역할(planner, content_organizer, layout_selector, editor, coder)을 분리하여 각각 30~60줄 프롬프트

**현재 slide_designer.md 실효성 분석**:
- 넷제로 10장 생성 시 **가이드 무시됨** — 직접 패턴 함수 코딩
- 원인: 가이드가 나빠서가 아니라 **컴포넌트가 없어서 가이드를 따를 수 없었음**
- 지금은 42개 컴포넌트가 있으므로 **가이드를 따를 환경이 됨**

### 6.2 원안 vs 1차 수정 vs 최종안 비교

| 항목 | 원안 | 1차 수정 | 최종안 | 최종 근거 |
|------|------|---------|-------|---------|
| 가이드 구조 | 플랫 테이블 유지 | 계층형 트리 + 예시 | **테이블 유지 + 예시 5개 + 자기검증** | PPTAgent가 플랫 리스트로 충분, 트리는 과잉 |
| 가이드 규모 | ~120줄 | ~200줄 | **~150줄** | 현재 116줄에서 컴포넌트 교체 + 예시/체크 추가 |
| 스코어링 | 5요소 공식 | IF-THEN 규칙 | **예시 기반 패턴 매칭** | LLM은 공식도 규칙도 아닌 예시를 가장 잘 따름 |
| SEQUENCE 단계 | 별도 삽입 | PLAN에 통합 | **PLAN에 규칙 3개 통합** (확정) | 변경 없음 |
| evaluate.py | 체크 3개 추가 | 체크 2개 추가 | **체크 2개 추가** (확정) | 변경 없음 |
| RECIPES | 전면 교체 | 메타데이터 추가 | **최소 정비** (컴포넌트명 교체만) | 과도한 구조화 불필요 |
| 피드백 루프 | 코드 구현 | 메모리 활용 | **메모리 활용** (확정) | 변경 없음 |
...
```
Claude Code가 덱 생성 시:
  1. slide_designer.md를 읽음 (패턴 이름만 있음)
  2. 패턴 이름을 보고 pattern 함수를 호출 → 1패턴 = 1슬라이드
  3. 10장이 다 비슷한 느낌
  4. evaluate.py는 텍스트 밀도/폰트만 체크
```

**After (Phase 3 완료 후)**:
```
Claude Code가 덱 생성 시:
  1. slide_designer.md를 읽음 (컴포넌트 조합 + 5개 실전 예시)
  2. 예시를 보고 "이 콘텐츠는 저 예시와 비슷하니 이 조합을 쓰자" 판단
  3. SlideComposer + comp_xxx() 코드 작성 → 매 슬라이드 다른 조합
  4. 자기검증 4개로 오류 사전 방지
  5. PLAN에서 리듬 규칙 적용 → 연속 동일 방지
  6. evaluate.py가 공간 활용률 + 덱 유사도도 체크
```

### 6.4 왜 이 안이 더 나은 산출물을 만드는가

1. **예시 기반**: "이런 콘텐츠면 이렇게 조합했다"를 5개 보여주면, Claude Code는 새 콘텐츠에 대해 가장 비슷한 예시를 찾아 변형합니다. 규칙 20개보다 예시 5개가 효과적 (LLM 리서치 확인).

2. **최소 변경**: 현재 가이드의 구조(테이블)를 유지하고 값만 교체하므로, 기존에 작동하던 부분이 깨지지 않습니다.

3. **자기검증**: 비용 제로로 "직전 슬라이드와 같은 레이아웃인가?" 같은 명백한 실수를 방지합니다.

4. **기계적 가드레일**: evaluate.py의 2개 체크가 Claude의 판단을 보완합니다.
