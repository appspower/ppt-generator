"""HD현대 SAP S/4HANA 전환 제안서 — 워크플로우 6단계 준수 버전.

07_WORKFLOW.md의 6단계 프로세스를 명시적으로 따름:
Step 1: ANALYZE — 리서치, 핵심 메시지 5개, 관점 분류
Step 2: PLAN — 슬라이드별 purpose, content_type, item_count
Step 3: SELECT — 메타데이터 기반 패턴 매칭 (같은 패턴 연속 사용 금지)
Step 4: GENERATE — 선택된 패턴으로 렌더링
Step 5: EVALUATE — validate_visual + design_check
Step 6: REFINE — 이슈 있으면 수정 후 재렌더
"""
...
def build_deck():
    ...
    # ================================================================
    # 7. Quadrant Story (투자 우선순위) — Step 3 SELECT: environment_scan → quadrant_story
    # ================================================================
    quadrant_story(add_slide(prs), QuadrantSpec(
        ...
        intro="단기 ROI와 구현 난이도 두 축으로 8개 모듈 분류 — Quick Win 선행 후 Strategic Bet 단계적 확대",
...
def main():
    out_dir = Path("output/full_deck_workflow")
    out_dir.mkdir(parents=True, exist_ok=True)
    out = out_dir / "HD현대_SAP_전환_제안서_워크플로우.pptx"

    print("=" * 70)
    print("HD현대 SAP 전환 제안서 — 워크플로우 6단계 준수")
    print("=" * 70)
